...
class coordinateDescentLassoRegression(coordinateDescentRegression):
    def __init__(self, X, y, lambda_=0.01, numberIterations=50):
        super(coordinateDescentLassoRegression,
              self).__init__(X, y, numberIterations)
    ...
    def updateTheta(self):
        ...
        for j in range(self.features + 1):
            ...
            temp1 = epsilon + np.matmul(
                self.augmentedX[:, j].reshape(self.augmentedX.shape[0], 1),
                currentTheta[j].reshape(currentTheta.shape[1], 1))
...
class gradientDescentAutogradLassoRegression(gradientDescentAutogradRegression
    ...
    def __init__(self,
                 ...
                 numberIterations=50,
                 lambda_=0.01):
        super(gradientDescentAutogradLassoRegression,
    ...
    def trainingLoss(self, theta):
        ...
        mse = (np.sum(epsilon**2) +
               self.lambda_ * np.abs(theta)) / self.sampleCount
        return mse