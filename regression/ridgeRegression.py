...
class normalEquationRidgeRegression(normalEquationRegression):
    ...
    def train(self):
        ...
        t1 = np.linalg.inv(
            tempX.T.dot(tempX) + np.absolute(self.lambda_) *
            (np.identity(tempX.shape[1])))
        t2 = t1.dot(tempX.T)
...
class gradientDescentRidgeRegression(gradientDescentRegression):
    def __init__(self,
                 ...
                 numberIterations=50,
                 lambda_=0.01):
        super(gradientDescentRidgeRegression,
    ...
    def updateTheta(self):
        ...
        differential = self.augmentedX.T.dot(epsilon) - np.absolute(
...
class gradientDescentAutogradRidgeRegression(gradientDescentAutogradRegression
    ...
    def __init__(self,
                 ...
                 numberIterations=50,
                 lambda_=0.01):
        super(gradientDescentAutogradRidgeRegression,
              self).__init__(X, y, alpha, numberIterations)
        self.lambda_ = lambda_
    ...
    def trainingLoss(self, theta):
        ...
        mse = (np.sum(epsilon**2) +
               self.lambda_ * np.matmul(theta.T, theta)) / self.sampleCount
        return mse